...
if (typeof window.GPApiConfig === "undefined") {
  window.GPApiConfig = {
    endpoints: {
      chat: "/api/chat",
      reports: "/api/reports"
    ...
    resolveEndpoint: async function(endpointKey) {
      ...
      // 2. If local is down or we are running in production (Netlify / Render Static), route directly to the production base URL (Render / Cloudflare Worker)
